...
sk_sp<DlImage> EmbedderExternalTextureVK::ResolveTexture(
  ...
  if (!external ||
      external->struct_size < sizeof(FlutterVulkanExternalTexture) ||
      !external->destruction_callback) {
    FML_LOG(ERROR) << "Invalid Vulkan external texture ownership metadata.";
    return nullptr;
  }
