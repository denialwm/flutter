...
bool ContextVK::BeginFrameImages(
  ...
  frame_images_.clear();
  frame_images_.push_back(
      {std::move(root_image), external_layout, external_queue_family, true});
  frame_image_state_ = FrameImageState::kOpen;
  return true;
}

bool ContextVK::AddFrameTexture(std::shared_ptr<const TextureSourceVK> texture,
                                vk::ImageLayout external_layout,
                                uint32_t external_queue_family) {
  if (frame_image_state_ != FrameImageState::kOpen || !texture ||
      external_layout == vk::ImageLayout::eUndefined ||
      external_layout == vk::ImageLayout::ePreinitialized ||
      !SupportsExternalQueueFamily(external_queue_family)) {
    return false;
  ...
  const auto existing = std::find_if(frame_images_.begin(), frame_images_.end(),
                                     [image](const FrameImage& entry) {
                                       return entry.source->GetImage() == image;
                                     });
  if (existing != frame_images_.end()) {
...
bool ContextVK::AcquireFrameImages() {
  if (frame_image_state_ != FrameImageState::kOpen || frame_images_.empty()) {
    return false;
  }
  ...
  frame_image_state_ = FrameImageState::kAcquired;
  return true;
}

std::shared_ptr<CommandBuffer> ContextVK::CreateFrameReleaseCommandBuffer() {
  if (frame_image_state_ != FrameImageState::kAcquired ||
      frame_images_.empty()) {
    return nullptr;
  }
  auto command = CreateCommandBuffer();
...
bool ContextVK::CancelFrameImages() {
  if (frame_image_state_ == FrameImageState::kClosed) {
    return true;
  }
  bool restored = true;
  if (frame_image_state_ == FrameImageState::kAcquired) {
    const bool flushed = FlushCommandBuffers();
    auto release = CreateFrameReleaseCommandBuffer();
    bool released = false;
    if (release) {
    ...
    if (restored) {
      ...
      VALIDATION_LOG << "Could not release a cancelled external frame.";
    }
    if (!flushed) {
      VALIDATION_LOG << "Could not flush a cancelled external frame.";
    }
  }
  frame_images_.clear();
  frame_image_state_ = FrameImageState::kClosed;
  return restored;
}

bool ContextVK::SubmitFrameFence(fml::UniqueFD& fd) {
#if defined(FML_OS_LINUX) || defined(FML_OS_ANDROID)
  if (!SupportsFrameFence()) {
    VALIDATION_LOG << "External sync-file semaphores are unavailable.";
    return false;
  }
  if (!FlushCommandBuffers()) {
    return false;
  }
  auto release = CreateFrameReleaseCommandBuffer();
  if (!release) {
    VALIDATION_LOG << "No acquired external images to release.";
    return false;
  }
  auto& release_vk = CommandBufferVK::Cast(*release);
  if (!release_vk.EndCommandBuffer()) {
  ...
  if (fence_result != vk::Result::eSuccess) {
    VALIDATION_LOG << "Could not create frame-completion fence: "
  ...
  auto submit_result = GetGraphicsQueue()->Submit(submit_info, *fence);
  if (submit_result != vk::Result::eSuccess) {
    VALIDATION_LOG << "Could not submit frame-completion signal: "
...
bool ContextVK::SupportsFrameFence() const {
#if defined(FML_OS_LINUX) || defined(FML_OS_ANDROID)
